...
ฅ^•ﻌ•^ฅ --------------------------- SECTION 3 --------------------------- ฅ^•ﻌ•^ฅ
                    ...
                    make changes to buffer content. In normal mode, there are
                    five frequently used keys for editing: ~c, d, p, s, u~.

    .---, .---,     Example: to change a word, select it first (~e~) and then
    │ e │+│ c │     change (~c~) it by typing in the new word. Of course, you
    `---' `---'     can expand the selection and then change a bunch of words.

    .---, .---,     Similarly, to change a whole line, select it with ~x~ and
    │ x │+│ c │     then change (~c~) it. Again, Of course, you
    `---' `---'     can expand the selection and then change a bunch of words.

       .---,        If you want to delete the selected object (word, phrase or
       │ s │        line), press ~s~ (~meow-kill~). For Emacs user, to kill
       `---'        means to cut. Therefore, to yank (or paste) the just-killed
                    text, type ~p~ (~meow-yank~). If you later changed your mind
                    and decide to discard the changes, hit ~u~ (~meow-undo~) a
                    few times to go back.
